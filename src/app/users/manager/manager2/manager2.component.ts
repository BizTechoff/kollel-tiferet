...
@Component({
  ...
  styleUrls: ['./manager2.component.scss']
})
export class Manager2Component implements OnInit {
  ...
  async save() {
    if (!this.selectedBranch) {
      ...
      if (isNew) {
        ...
        if (this.selectedBranch?.id !== this.bid) {
          let ub = await remult.repo(UserBranch).findId(this.ubs[0])
          if (ub) {
  ...
  async removeSecondBranch(e: any) {
    e?.stopPropagation()
    let yes = await this.ui.yesNoQuestion(`?להסיר את הכולל השני`)
    if (yes) {
  ...
  async addBranch(bid = '', uid = '', second = false) {
    ...
    if (bid?.trim().length) {
      ...
      if (ub) {
        ...
        if (!this.ubs.includes(ub.id)) {
          this.ubs.push(ub.id)
        }
        this.bid = ub.branch?.id
      }
    }
  }
